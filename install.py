...
def install_app(venv_path):
    print("Installing Scramble Generator into the virtual environment...", end="")
    sys.stdout.flush()
    pip_path = os.path.join(venv_path, "bin", "pip")
    subprocess.run(
        [pip_path, "install", "scramble_generator"],
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
...
def get_icon(venv_path):
    ...
    icon_relative_path = (
        "scramble_generator/resources/images/scramble_generator-128.png"
    )
...
def create_desktop_file(icon, version, python, app):
    ...
    Comment=Python app for generating scrambles for twisty puzzles.
    Exec={python} {app}
    Icon={icon}
    Terminal=false
    Categories=Utility;
    ...
    with open(
        os.path.expanduser("~/.local/share/applications/scramble_generator.desktop"),
        "w",
    ) as f:
